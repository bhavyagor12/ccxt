...
class cow extends Exchange {
    ...
    public function fetch_balance($params = array ()): PromiseInterface {
        return Async\async(function () use ($params) {
            ...
            for ($i = 0; $i < count($currencyCodes); $i++) {
                ...
                try {
                    $balanceRaw = Async\await($this->fetch_erc20_balance($tokenAddress, $ownerAddress));
        ...
        return Async\async(function () use ($symbol, $type, $side, $amount, $price, $params) {
            ...
            if ($autoApprove) {
                ...
                if ($sellTokenAddress !== null) {
                    ...
                    try {
                        $currentAllowance = Async\await($this->check_erc20_allowance($sellTokenAddress, $this->hex_with0x_prefix($signerAddr), $vaultRelayer));
                        if (Precise::string_lt($currentAllowance, $requiredAmount)) {
                            $this->log('Insufficient allowance detected. Approving VaultRelayer...');
                            $approvalResult = Async\await($this->approve_erc20($sellTokenAddress, $vaultRelayer));
                            $this->log('Approval transaction:', $approvalResult['txHash']);
                            Async\await($this->sleep(5000));
                        }
                    } catch (Exception $e) {
    ...
    public function cancel_order(string $id, ?string $symbol = null, $params = array ()): PromiseInterface {
        return Async\async(function () use ($id, $symbol, $params) {
            ...
            $response = Async\await($this->request('api/v1/orders', 'public', 'DELETE', $params, $headers, $this->json($requestBody)));
    ...
    public function cancel_all_orders(?string $symbol = null, $params = array ()): PromiseInterface {
        return Async\async(function () use ($symbol, $params) {
             ...
             */
            Async\await($this->load_markets());
            $openOrders = Async\await($this->fetch_open_orders($symbol, null, null, $params));
            ...
            for ($i = 0; $i < count($openOrders); $i++) {
                ...
                if ($orderId !== null) {
                    try {
                        $result = Async\await($this->cancel_order($orderId, $symbol, $params));
                        $results[] = $result;
                    } catch (Exception $e) {
    ...
    public function fetch_erc20_balance(?string $tokenAddress, ?string $ownerAddress) {
        return Async\async(function () use ($tokenAddress, $ownerAddress) {
            ...
            $response = Async\await($this->fetch($rpcUrl, 'POST', array( 'Content-Type' => 'application/json' ), $this->json($request)));
    ...
    public function check_erc20_allowance(?string $tokenAddress, ?string $ownerAddress, ?string $spenderAddress) {
        return Async\async(function () use ($tokenAddress, $ownerAddress, $spenderAddress) {
            $rpcUrl = $this->get_rpc_url_option();
            if ($rpcUrl === null) {
    ...
    public function approve_erc20(?string $tokenAddress, ?string $spenderAddress, ?string $amount = null) {
        return Async\async(function () use ($tokenAddress, $spenderAddress, $amount) {
            $rpcUrl = $this->get_rpc_url_option();
            if ($rpcUrl === null) {
            ...
            $nonceResponse = Async\await($this->fetch($rpcUrl, 'POST', array( 'Content-Type' => 'application/json' ), $this->json($nonceRequest)));
            ...
            $gasPriceResponse = Async\await($this->fetch($rpcUrl, 'POST', array( 'Content-Type' => 'application/json' ), $this->json($gasPriceRequest)));
            ...
            $broadcastResponse = Async\await($this->fetch($rpcUrl, 'POST', array( 'Content-Type' => 'application/json' ), $this->json($broadcastRequest)));
