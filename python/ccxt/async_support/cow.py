...
class cow(Exchange, ImplicitAPI):
    ...
    async def fetch_balance(self, params={}) -> Balances:
        ...
        for i in range(0, len(currencyCodes)):
            ...
            try:
                balanceRaw = await self.fetch_erc20_balance(tokenAddress, ownerAddress)
    ...
    async def create_order(self, symbol: str, type: OrderType, side: OrderSide, amount: float, price: float = None, params={}) -> Order:
        ...
        if autoApprove:
            ...
            if sellTokenAddress is not None:
                ...
                try:
                    currentAllowance = await self.check_erc20_allowance(sellTokenAddress, self.hex_with0x_prefix(signerAddr), vaultRelayer)
                    if Precise.string_lt(currentAllowance, requiredAmount):
                        self.log('Insufficient allowance detected. Approving VaultRelayer...')
                        approvalResult = await self.approve_erc20(sellTokenAddress, vaultRelayer)
    ...
    async def cancel_order(self, id: str, symbol: Str = None, params={}) -> Order:
        ...
        headers = {'Content-Type': 'application/json'}
        response = await self.request('api/v1/orders', 'public', 'DELETE', params, headers, self.json(requestBody))
    ...
    async def cancel_all_orders(self, symbol: Str = None, params={}) -> List[Order]:
        ...
        """
        await self.load_markets()
        openOrders = await self.fetch_open_orders(symbol, None, None, params)
        if len(openOrders) == 0:
            return []
        results = []
        for i in range(0, len(openOrders)):
            ...
            if orderId is not None:
                try:
                    result = await self.cancel_order(orderId, symbol, params)
    ...
    async def fetch_erc20_balance(self, tokenAddress: Str, ownerAddress: Str):
        ...
        response = await self.fetch(rpcUrl, 'POST', {'Content-Type': 'application/json'}, self.json(request))
    ...
    async def check_erc20_allowance(self, tokenAddress: Str, ownerAddress: Str, spenderAddress: Str):
        rpcUrl = self.get_rpc_url_option()
        if rpcUrl is None:
    ...
    async def approve_erc20(self, tokenAddress: Str, spenderAddress: Str, amount: Str = None):
        rpcUrl = self.get_rpc_url_option()
        if rpcUrl is None:
        ...
        nonceResponse = await self.fetch(rpcUrl, 'POST', {'Content-Type': 'application/json'}, self.json(nonceRequest))
        ...
        gasPriceResponse = await self.fetch(rpcUrl, 'POST', {'Content-Type': 'application/json'}, self.json(gasPriceRequest))
        ...
        broadcastResponse = await self.fetch(rpcUrl, 'POST', {'Content-Type': 'application/json'}, self.json(broadcastRequest))
