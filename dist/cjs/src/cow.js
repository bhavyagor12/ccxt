...
var address = require('./static_dependencies/ethers/address/address.js');
...
class cow extends cow$1["default"] {
    ...
    async createOrder(symbol, type, side, amount, price = undefined, params = {}) {
        ...
        let sellAmountRaw = undefined;
        let buyAmountRaw = undefined;
        if (isSellOrder) {
        ...
        if (autoApprove) {
            ...
            if (sellTokenAddress !== undefined) {
                ...
                const requiredAmount = Precise["default"].stringAdd(sellAmount, feeAmount);
                try {
                    const currentAllowance = await this.checkERC20Allowance(sellTokenAddress, this.hexWith0xPrefix(signerAddr), vaultRelayer);
                    if (Precise["default"].stringLt(currentAllowance, requiredAmount)) {
                    ...
                    throw new errors.ExchangeError(this.id + ' createOrder() failed to ensure allowance: ' + message);
                }
            }
        }
    ...
    ensureOwnerAddress(params = {}) {
        ...
        if ((derivedOwner !== undefined) && (owner !== undefined) && (owner.toLowerCase() !== derivedOwner)) {
            throw new errors.AuthenticationError(this.id + ' walletAddress mismatch between provided owner and private key');
        }
        if (owner === undefined) {
    ...
    deriveWalletAddressFromPrivateKey() {
        ...
        if (keyHex.length !== 64) {
            throw new errors.AuthenticationError(this.id + ' privateKey must be a 32-byte hex string');
        }
        const privateKeyBytes = this.base16ToBinary(keyHex);
        const publicKey = secp256k1.secp256k1.getPublicKey(privateKeyBytes, false);
        const hashBytes = sha3.keccak_256(publicKey.slice(1));
    ...
    amountToTokenAmount(amountString, decimals) {
        ...
        const decimalsString = this.numberToString(decimals);
        const precision = this.parsePrecision(decimalsString);
        return Precise["default"].stringDiv(amountString, precision);
    }
    getChainIdOption() {
    ...
    getVaultRelayerOption() {
        ...
        if (vaultRelayer === undefined) {
            throw new errors.ExchangeError(this.id + ' unsupported network ' + network + ' for vault relayer resolution');
        }
    ...
    async fetchERC20Balance(tokenAddress, ownerAddress) {
        const rpcUrl = this.getRpcUrlOption();
        if (rpcUrl === undefined) {
            throw new errors.ExchangeError(this.id + ' fetchERC20Balance() requires rpcUrls[' + this.options['network'] + '] to be configured');
        }
        ...
        if (errorResult !== undefined) {
            throw new errors.ExchangeError(this.id + ' fetchERC20Balance() RPC error: ' + this.json(errorResult));
        }
        if (result === '0x') {
            return '0';
        }
    ...
    async checkERC20Allowance(tokenAddress, ownerAddress, spenderAddress) {
        const rpcUrl = this.getRpcUrlOption();
        if (rpcUrl === undefined) {
            throw new errors.ExchangeError(this.id + ' checkERC20Allowance() requires rpcUrls[' + this.options['network'] + '] to be configured');
        }
        const ownerPadded = this.remove0xPrefix(ownerAddress).padStart(64, '0');
        ...
        if (result === '0') {
            const errorResult = this.safeValue(response, 'error');
            if (errorResult !== undefined) {
                throw new errors.ExchangeError(this.id + ' checkERC20Allowance() failed to fetch allowance: ' + this.json(response));
            }
        }
        return result;
    }
    async approveERC20(tokenAddress, spenderAddress, amount = undefined) {
        const rpcUrl = this.getRpcUrlOption();
        if (rpcUrl === undefined) {
            throw new errors.ExchangeError(this.id + ' approveERC20() requires rpcUrls[' + this.options['network'] + '] to be configured');
        }
        ...
        const txHash = '0x' + this.binaryToBase16(sha3.keccak_256(this.base16ToBinary(this.remove0xPrefix(rlpEncoded))));
        ...
        const signature = secp256k1.secp256k1.sign(hashBytes, privateKeyBytes);
        ...
        if (error !== undefined) {
            throw new errors.ExchangeError(this.id + ' approveERC20() failed: ' + this.json(error));
        }
        return {
            'txHash': txHashResult,
            'token': tokenAddress,
    ...
    computeTypedDataDigest(domain, types, message) {
        const encoder = typedData.TypedDataEncoder;
        ...
        const digestBytes = sha3.keccak_256(encodedBinary);
        return '0x' + this.binaryToBase16(digestBytes);
    }
    hashEthereumSignedMessage(messageHex) {
        if (messageHex === undefined) {
            return undefined;
        }
        const binaryMessage = this.base16ToBinary(this.remove0xPrefix(messageHex));
        if (binaryMessage.length !== 32) {
            throw new errors.ExchangeError(this.id + ' hashEthereumSignedMessage() requires a 32-byte digest hex string');
        }
        ...
        const digestBytes = sha3.keccak_256(prefixed);
        return '0x' + this.binaryToBase16(digestBytes);
    }
    signDigest(digest, privateKey, usePersonalSign = false) {
        ...
        const signature = secp256k1.secp256k1.sign(digestBytes, privateKeyBytes);
        const compact = signature.toCompactHex();
        const recovery = signature.recovery;
        const r = compact.slice(0, 64);
        const s = compact.slice(64);
    ...
    signOrderPayload(order, signingScheme = 'eip712') {
        const privateKey = this.normalizePrivateKey(this.privateKey);
        const chainId = this.getChainIdOption();
        ...
        if (receiverAddress !== undefined) {
            normalizedReceiver = address.getAddress(this.hexWith0xPrefix(receiverAddress));
        }
        const message = {
            'sellToken': address.getAddress(this.hexWith0xPrefix(order['sellToken'])),
            'buyToken': address.getAddress(this.hexWith0xPrefix(order['buyToken'])),
        ...
        const typedDataEncoder = new typedData.TypedDataEncoder(types);
        const domainSeparator = typedData.TypedDataEncoder.hashDomain(domain);
        const structHash = typedDataEncoder.hash(message);
        const prefix = '0x1901';
        const encoded = prefix + domainSeparator.slice(2) + structHash.slice(2);
        const digest = '0x' + this.binaryToBase16(sha3.keccak_256(this.base16ToBinary(encoded.slice(2))));
        const usePersonalSign = (signingScheme === 'ethsign');
        return this.signDigest(digest, privateKey, usePersonalSign);
    }
    signOrderCancellation(orderUids, signingScheme = 'eip712') {
        const privateKey = this.normalizePrivateKey(this.privateKey);
        const chainId = this.getChainIdOption();
        const verifyingContract = this.getVerifyingContractOption();
        const domain = {
        ...
        const typeHash = '0x' + this.binaryToBase16(sha3.keccak_256(typeString));
        ...
        for (let i = 0; i < orderUids.length; i++) {
            ...
            const uidHash = sha3.keccak_256(uidBytes);
            concatenatedHashes += this.binaryToBase16(uidHash);
        }
        const arrayHash = '0x' + this.binaryToBase16(sha3.keccak_256(this.base16ToBinary(concatenatedHashes)));
        const structHashInput = typeHash.slice(2) + arrayHash.slice(2);
        const structHash = '0x' + this.binaryToBase16(sha3.keccak_256(this.base16ToBinary(structHashInput)));
        const domainSeparator = typedData.TypedDataEncoder.hashDomain(domain);
        const prefix = '0x1901';
        const encoded = prefix + domainSeparator.slice(2) + structHash.slice(2);
        const digest = '0x' + this.binaryToBase16(sha3.keccak_256(this.base16ToBinary(encoded.slice(2))));
